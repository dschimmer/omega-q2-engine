...
// XXX: Note i made this static as we have a collision with qgl_linux, probably because we're hardlinking in this case....ugh....
static const char so_file[] = "/etc/quake2.conf";
...
memset(boobs, 0, sizeof(boobs));

readlink("/proc/self/exe", boobs, sizeof(boobs) - 1);

Com_Printf("hooters!!!! %s", boobs);


	if ( ( reflib_library = dlopen( "data/data/com.example.quake2/lib/libref_gles.so", RTLD_LAZY | RTLD_GLOBAL ) ) == 0 )
	{
		Com_Printf( "LoadLibrary(\"%s\") failed: %s\n", name , dlerror());
		return false;
	}
...
// XXX: Not ready yet.....
	...
	Real_IN_Init();
Com_Printf( "Calling render api init, mainwin %p\n", mainWindow);
	if ( re.Init(mainWindow, 0 ) == -1 )
	{
		re.Shutdown();
		VID_FreeReflib ();
		return false;
...
void VID_CheckChanges (void)
{
	...
	while (vid_ref->modified)
	{
		...
		*/
		...
		sprintf( name, "libref_%s.so", vid_ref->string );
...
void VID_Init (void)
{
    ...
    vid_ref = Cvar_Get ("vid_ref", "gles", CVAR_ARCHIVE);
