...
void DrawGLPoly (glpoly_t *p)
{
    ...
    qglVertexPointer(3, GL_FLOAT, VERTEXSIZE * sizeof(float), &p->verts[0][0]);
    ...
    qglBindBuffer(GL_ARRAY_BUFFER, p->glbuffer);
...
void DrawGLFlowingPoly (msurface_t *fa)
{
    ...
    qglEnableClientState(GL_VERTEX_ARRAY);
    qglVertexPointer(3, GL_FLOAT, VERTEXSIZE * sizeof(float), &p->verts[0][0]);
    // XXX: Take into account some scroll...
    qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
    qglTexCoordPointer(2, GL_FLOAT, VERTEXSIZE * sizeof(float), &p->verts[0][3]);
...
void DrawGLPolyChain(glpoly_t *p, float soffset, float toffset)
{
    if ( soffset == 0 && toffset == 0 )
    {
        for ( ; p != 0; p = p->chain )
        {
            ...
            qglTexCoordPointer(2, GL_FLOAT, VERTEXSIZE * sizeof(float), p->verts[5]);
...
// XXX:        GL_EnableMultitexture(false);
        for (i = 0, image = gltextures; i < numgltextures; i++, image++) {
            if (!image->registration_sequence) {
                continue;
            }
...
//    qboolean is_dynamic = false;
    ...
    glpoly_t *p = NULL;
// no more of this dynamic style stuff, moved to different stage....
#if 0
...
//PGM
        if (surf->texinfo->flags & SURF_FLOWING) {
            float scroll;
Com_Printf("Got a surface flowing!!\n");
            scroll = -64 * ( (r_newrefdef.time / 40.0) - (int)(r_newrefdef.time / 40.0) );
            if (scroll == 0.0) {
                scroll = -64.0;
            }
...
// reconstruct the polygon
    ...
    // XXX: Experimental, bind the buffer....
    qglGenBuffers(1, &poly->glbuffer);
    qglBindBuffer(GL_ARRAY_BUFFER, poly->glbuffer);
    
    qglBufferData(GL_ARRAY_BUFFER, lnumverts * VERTEXSIZE * sizeof(float), poly->verts, GL_STATIC_DRAW);
