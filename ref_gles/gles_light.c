...
void R_RenderDlight(dlight_t *light)
{
    ...
    *pVert++ = light->color[0] * 0.2;
    *pVert++ = light->color[1] * 0.2;
    *pVert++ = light->color[2] * 0.2;
    *pVert++ = 1.0f;

    for (i = 16; i >= 0; i--) {
        a = i / 16.0 * M_PI * 2;
        for (j = 0; j < 3; j++) {
            *pVert++ = light->origin[j] + vright[j] * cos(a) * rad + vup[j] * sin(a) * rad;
        }

        *pVert++ = 0; //light->color[0] * 0.2;
        *pVert++ = 0; //light->color[1] * 0.2;
        *pVert++ = 0; //light->color[2] * 0.2;
        *pVert++ = 1.0;
    }

    qglEnableClientState(GL_VERTEX_ARRAY);
    qglVertexPointer(3, GL_FLOAT, 7 * sizeof(float), &vertices);

    qglEnableClientState(GL_COLOR_ARRAY);
    qglColorPointer(4, GL_FLOAT, 7 * sizeof(float), &vertices[4]);
...
void R_RenderDlights(void)
{
    ...
    // don't bother changing states if there is nothing to do.....
