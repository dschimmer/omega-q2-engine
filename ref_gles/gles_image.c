...
void GL_SelectTexture(int32_t unit)
{
    ...
    if (unit == 0) {
        ...
        ri.Sys_Error(ERR_DROP, "GL_SelectTexture: unit = %d", unit);
    }
    
    gl_state.currenttmu = unit;
...
{
    GL_SelectTexture(target == GL_TEXTURE0 ? 0 : 1);
...
void GL_MBind(int32_t unit, int32_t texnum)
{
    GL_SelectTexture(unit);

    if (gl_state.currenttextures[unit] == texnum) {
        return;
    }
...
//GL_LightScaleTexture(scaled, scaled_width, scaled_height, false);
    ...
    GL_LightScaleTexture(data, width, height, false);
    ...
    qglTexImage2D(GL_TEXTURE_2D, 0, comp, width, height, 0, comp, GL_UNSIGNED_BYTE, data);
